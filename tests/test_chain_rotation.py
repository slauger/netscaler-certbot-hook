...
def auth_headers():
    ...
    return {"X-NITRO-USER": "nsroot", "X-NITRO-PASS": "nsroot", "Content-Type": "application/json"}
...
def test_chain_rotation_without_unlink_fails(mock_server, auth_headers):
    ...
    response = requests.get(
        f"{base_url}/nitro/v1/config/sslcertkey/example.com", headers=auth_headers
    )
    ...
    response = requests.post(
        f"{base_url}/nitro/v1/config/sslcertkey?action=link",
        json={"sslcertkey": {"certkey": "example.com", "linkcertkeyname": "E7"}},
        headers=auth_headers,
    )
...
def test_chain_rotation_with_unlink_succeeds(mock_server, auth_headers):
    ...
    requests.post(
        f"{base_url}/nitro/v1/config/sslcertkey",
        json={
            "sslcertkey": {"certkey": "E6", "cert": "/nsconfig/ssl/E6-123.crt", "serial": "0xabc"}
        },
        headers=auth_headers,
    )
    requests.post(
    ...
    response = requests.post(
        f"{base_url}/nitro/v1/config/sslcertkey",
        json={
            "sslcertkey": {"certkey": "E7", "cert": "/nsconfig/ssl/E7-456.crt", "serial": "0xdef"}
        },
        headers=auth_headers,
    )
    assert response.status_code == 200, f"Failed to add E7: {response.text}"
...
def test_authentication_required(mock_server):
    ...
    response = requests.get(
        ...
        headers={"X-NITRO-USER": "nsroot", "X-NITRO-PASS": "wrongpass"},
    )
    assert response.status_code == 401
