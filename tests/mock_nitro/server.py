...
def error_response(error_code: int, message: str, status: int = 400) -> Tuple[Dict[str, Any], int]:
    ...
    """
    return jsonify({"errorcode": error_code, "message": message, "severity": "ERROR"}), status
...
def success_response(data: Dict[str, Any] = None) -> Tuple[Dict[str, Any], int]:
    ...
    if data:
        ...
        return jsonify({"errorcode": 0, "message": "Done", "severity": "NONE"}), 200
...
def check_nitro_auth(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not username or not password:
            ...
            return error_response(-1, "Username and password not specified", 401)

        # Validate credentials
        if username not in state.USERS or state.USERS[username] != password:
            logger.warning(f"Failed authentication attempt for user: {username}")
            return error_response(354, "Invalid username or password", 401)
...
def list_certificates():
    ...
    return jsonify({"sslcertkey": list(certs.values())})
...
def get_certificate(certkey: str):
    ...
    if not cert:
        ...
        return error_response(258, "No such resource", 404)

    return jsonify({"sslcertkey": [cert]})
...
def manage_certificate():
    ...
    if not data or "sslcertkey" not in data:
        return error_response(1094, "Invalid argument [sslcertkey]")

    cert_data = data["sslcertkey"]
    certkey = cert_data.get("certkey")

    if not certkey:
        return error_response(1094, "Invalid argument [certkey]")
...
def add_certificate(cert_data: Dict[str, Any]) -> Tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        serial_info = (
            f"with serial {stored_cert.get('serial')}"
            if stored_cert.get("serial")
            else "without serial"
        )
        ...
        if "already exists" in str(e):
            ...
            return error_response(273, f"Resource already exists [{certkey}]", 409)
        raise
...
def link_certificate(cert_data: Dict[str, Any]) -> Tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        if "not found" in error_msg:
            ...
            return error_response(1540, error_msg, 409)
        raise
...
def unlink_certificate(cert_data: Dict[str, Any]) -> Tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        if "not found" in error_msg:
            ...
            return error_response(1541, error_msg, 400)
        raise
...
def save_config():
    ...
    return error_response(1094, "Invalid action")
...
def upload_file():
    ...
    if not data or "systemfile" not in data:
        return error_response(1094, "Invalid argument [systemfile]")
    ...
    if not filename:
        return error_response(1094, "Invalid argument [filename]")

    # Store the file content for later serial extraction
    if filecontent:
        state.store_uploaded_file(filename, filecontent)
        logger.info(
            f"POST /nitro/v1/config/systemfile - uploaded: {filename} ({len(filecontent)} bytes)"
        )
    else:
